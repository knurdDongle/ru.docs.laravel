...
#### Примерное время для обновления 1-2 часа 

> {note} Все возможные критические изменения - задокументированы. Только некоторые из этих критических изменений могут повлиять на разные частях фреймворка.

### Обновление зависимостей

В вашем файле `composer.json` обновите зависимость `laravel/framework` до `5.4.*`. Кроме того, необходимо обновить `phpunit/phpunit` до `~5.0`.

#### Удаление файла скомпилированных сервисов

Если он существует, можно удалить `bootstrap/cache/compiled.php` файл. Он больше не используется платформой.

#### Очистка кэша

После обновления всех пакетов надо выполнить `php artisan view:clear`, чтобы избежать Blade - ошибок, связанных с удалением `Illuminate\View\Factory::getFirstLoop()`. Кроме того, надо выполнить `php artisan route:clear`, чтобы сбросить кэш роутов.
...
Laravel Passport `2.0.0` был выпущен, чтобы предоставить совместимость с Laravel 5.4 и библиотекой [Axios](https://github.com/mzabriskie/axios). Если вы обновляете с Laravel 5.3 и используете предварительно созданные компоненты Vue Passport, нужно удостовериться, что библиотека Axios глобально доступна для приложения `axios`.

#### Laravel Scout

Laravel Scout `3.0.0` был создан для совместимости с  Laravel 5.4.

#### Laravel Socialite

Laravel Socialite `3.0.0` был создан для совместимости с Laravel 5.4.

#### Laravel Tinker

Чтобы продолжить использовать Artisan команду `tinker` , нужно установить `laravel/tinker`:

    composer require laravel/tinker

Как только пакет будет установлен, надо  добавить `Laravel\Tinker\TinkerServiceProvider::class` в массив `providers` в конфигурационном файле `config/app.php`.
...
Раньше, вызывая метод `Gate::getPolicyFor($class)`, возникало исключение, это в том случае если политика не была найдена. В текущей версии, метод возвращает `null`, если политика не будет найдена для данного класса.  Если вы вызываете этот метод, измените его с учетом этого нюанса, проверкой на  `null`:
...
Если вы хотите передать в секцию не экранированный контент, необходимо указывать это с помощью традиционного "long form" стиля:
...
Если вы вручную переопределяете массив  `$bootstrappers` в своем HTTP или Console kernel, нужно переименовать вход с  `DetectEnvironment` на `LoadEnvironmentVariables`.

### Broadcasting

#### Связывание каналов

При установки заполнителей канала в Laravel 5.3 используется символ  '*' . В Laravel 5.4 нужно определить эти заполнители, используя синтаксис `{foo}`, наполняя их, как маршруты:
...
Метод`every` был переименован в `nth` , чтобы соответствовать названию метода, определенному в Lodash.

#### Метод `random`

Вызов `$collection->random(1)` теперь будет возвращать новый экземпляр коллекции с одним элементом. Ранее, это возращало единственный объект. Метод будет возвращать единственный объект, если  аргументы не переданы.

### Контейнер

#### Алиасы через `bind` / `instance`

В предыдущих версиях Laravel мы могли передать массив первым аргументом в  `bind` или `instance`, чтобы зарегистрировать псевдоним:

    $container->bind(['foo' => FooContract::class], function () {
        return 'foo';
    });

Однако это поведение было удалено в Laravel 5.4. Чтобы зарегистрировать псевдоним,  теперь нужно использовать метод  `alias`:

    $container->alias(FooContract::class, 'foo');

#### Связывание классов через косую черту

Связывание классов в контейнере через косую черту больше не поддерживаются. Эта особенность потребовала  существенное количество вызовов форматирования строки,  внутри контейнера. Вместо этого просто зарегистрируйте свои классы без косой черты:
...
Метод `make`, больше не принимает вторым аргументом массив. Эта особенность, указывала на код с "запашком", тем более можете всегда строить объект в другом отношении, который более интуитивен.

#### Resolving анонимных функций 

Методам контейнера `resolving`  и  `afterResolving`  нужно передать название класса или ключ,  первым аргументом:
...
Метод `share` был удален из контейнера. Он считался устаревшим и не упоминался в документации уже несколько лет. Если вы используете его, то необходимо поменять его название на метод `singleton`:
...
Если вы ссылаетесь на трейт `IlluminateConsoleAppNamespaceDetectortrait`, обновите свой код, чтобы ссылка изменилась на `IlluminateConsoleDetectsApplicationnamespace`.
