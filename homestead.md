...
Laravel Homestead - это официальный, предварительно упакованный образ Vagrant (бокс), который предоставляет замечательную среду разработки, без необходимости устанавливать PHP, веб-сервер и другое серверное ПО на вашей локальной машине. Больше не стоит беспокоиться о том, что вы можете испортить свою операционную систему! Боксы Vagrant полностью одноразовые. Если что-то пойдет не так, вы сможете уничтожить и пересоздать бокс за считанные минуты!

Homestead запускается на любых системах Windows, Mac и Linux, и включает в себя веб-сервер Nginx, PHP 7.1, MySQL, Postgres, Redis, Memcached, Node и другие вкусности, которые могут пригодиться вам для разработки потрясающих приложений на Laravel.

> {Примечание} Если вы используете Windows, Вам может потребоваться включить аппаратную виртуализацию (VT-x). Обычно она включается в вашем BIOS. Если вы используете Hyper-V на UEFI системе, вам дополнительно может потребоваться отключить Hyper-V для доступа к VT-x.
...
- Postgres
- Composer
- Node (С Yarn, PM2, Bower, Grunt, и Gulp)
- Redis
- Memcached
- Beanstalkd
...
Для использования провайдера VMware, вам потребуется приобрести как VMware Fusion / Workstation, так и [VMware Vagrant plug-in](https://www.vagrantup.com/vmware). Хотя это не бесплатно, VMware может предоставить быструю производительность для совместного доступа к папкам из коробки.

Для использования провайдера Parallels, вам потребуется установить [плагин Parallels Vagrant](https://github.com/Parallels/vagrant-parallels). Это бесплатно.
...
Вы можете установить Homestead простым клонированием репозитория. Желательно клонировать репозиторий в папку `Homestead` внутри вашей "домашней" директории, так как бокс Homestead будет служить хостом для всех ваших проектов Laravel:
...
После клонирования репозитория Homestead, запустите команду `bash init.sh` внутри директории Homestead, чтобы создать конфигурационный файл `Homestead.yaml`. Файл `Homestead.yaml` будет помещён в скрытую директорию `~/.homestead`:
...
Ключ `provider` в вашем файле `~/.homestead/Homestead.yaml` указывает какой провайдер Vagrant необходимо использовать: `virtualbox`, `vmware_fusion`, `vmware_workstation`, или `parallels`. Вы можете установить провайдер, который более предпочтительнее для вас:
...
В свойстве `folders` файла `Homestead.yaml` указаны все папки, к которым вы хотите предоставить доступ среде Homestead. Как только файлы будут изменены, они тут же будут синхронизированы между локальной машиной и средой Homestead. Вы можете настроить столько папок, сколько необходимо:
...
Если вы измените свойство `sites` после инициализации бокса Homestead, вы должны перезапустить `vagrant reload --provision` для обновления конфигурации Nginx на виртуальной машине.

#### Файл hosts

Вы должны добавить "домены" ваших Nginx сайтов в файл `hosts` на вашей машине. Файл `hosts` будет делать редирект запросов к вашим Homestead сайтам на вашу машину Homestead. На Mac и Linux этот файл находится в `/etc/hosts`. На Windows он находится в `C:\Windows\System32\drivers\etc\hosts`. Строки, добавленные в него, будут выглядеть следующим образом:

```
192.168.10.10  homestead.app
```

Убедитесь, что IP-адрес указанн в вашем файле `~/.homestead/Homestead.yaml`. После того как вы добавили домен в свой файл `hosts` и запустили бокс Vagrant, вы можете получить доступ к сайту через свой веб-браузер:
...
Вместо того, чтобы устанавливать Homestead глобально и использовать один и тот же бокс Homestead во всех ваших проектах, вы можете настроить экземпляр Homestead для каждого управляемого вами проекта. Установка Homestead для каждого проекта может быть полезна, если вы хотите расположить файл `Vagrantfile` внутри вашего проекта, позволяя другим работать над проектом, просто используя `vagrant up`.
...
После того, как Homestead был установлен, используйте команду `make` для генерации файлов `Vagrantfile` и `Homestead.yaml` в корне проекта. Команда `make` автоматически настроит директивы `sites` и `folders` в файле `Homestead.yaml`.
...
Затем запустите команду `vagrant up` в вашем терминале и получите доступ к вашему проекту через `http://homestead.app` в браузере. Помните, вам все равно следует добавить запись в файл `/etc/hosts` для `homestead.app` или другого домена по вашему выбору.
...
Убедитесь, что в данном примере путь `~/Homestead` в функции был изменен на фактическое местоположение с установленным Homestead. Как только функция установлена, вы можете запускать команды, например `homestead up` или `homestead ssh` из любого места вашей системы.
...
Убедитесь, что в данном примере путь `C:\Homestead` в скрипте был изменен на фактическое местоположение с установленным Homestead. После создания файла, добавьте его в переменную `PATH`. Затем вы можете запускать команды, такие как `homestead up` или `homestead ssh` из любого места вашей системы.
...
Так как вероятней всего, вам часто понадобится подключаться по SSH к вашей машине Homestead, подумайте о том, чтобы добавить "функцию", описанную выше, в вашу хостевую машину для быстрого доступа по SSH к боксу Homestead.
...
Laravel предоставляет удобный способ [планировки задач Cron](/docs/{{version}}/scheduling) с помощью одной команды Artisan `schedule:run`, настроенной на выполнение каждую минуту. Команда `schedule:run` изучает определённое в классе `App\Console\Kernel` расписание задач, чтобы понять, какие задачи следует запускать.

Если вы хотите настроить выполнение команды `schedule:run` на сайте в Homestead, то, при определении сайта, вам следует установить параметр `schedule` со значением `true`:
...
Вы можете обновить Homestead в два простых шага. Сначала вам следует обновить бокс Vagrant с помощью команды `vagrant box update`:
...
Вы можете легко изменить версию бокса Vagrant, который использует Homestead, добавив следующую строку в ваш файл `Homestead.yaml`:
...
При использовании старой версии бокса Homestead, вам необходимо убедиться в совпадении её версии с версией исходного кода Homestead. Ниже расположена таблица, показывающая поддерживаемые версии боксов Homestead, какую версию исходного кода стоит использовать, а также предоставляемую версию PHP:
